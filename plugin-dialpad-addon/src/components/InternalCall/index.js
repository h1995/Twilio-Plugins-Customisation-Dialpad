...
export const makeInternalCall = ({ manager, selectedWorker, workerList }) => {
	const workflow_sid = process.env.REACT_APP_WORKFLOW_SID;
	const queue_sid = process.env.REACT_APP_TASK_QUEUE_SID;
	// const {
	//     workflow_sid,
	//     queue_sid
	...
	const { attributes: { contact_uri, full_name: fromFullName }, name: fromName } = manager.workerClient;

	const { attributes: { full_name }, friendly_name } = workerList.find(
		(worker) => worker.attributes.contact_uri === selectedWorker
	);

	manager.workerClient.createTask(selectedWorker, contact_uri, workflow_sid, queue_sid, {
		attributes: {
			to: selectedWorker,
			direction: 'outbound',
